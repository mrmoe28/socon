...
const Navbar = ({ onLoginClick, onSettingsClick }: NavbarProps) => {
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${isScrolled ? 'bg-industrial-900/90 backdrop-blur-md border-zinc-800 py-3' : 'bg-transparent border-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-zinc-400">
          ...
          {isAuthenticated ? (
            <ProfileDropdown onSettingsClick={onSettingsClick} />
          ) : (
            <button 
              onClick={onLoginClick}
              className="bg-white text-black px-5 py-2 font-bold hover:bg-zinc-200 transition-colors skew-x-[-6deg]"
            >
